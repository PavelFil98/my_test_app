...
1.Реализован основной функциолнал веб-приложения(работает с PostgreSQL), 2. Для запуска используется Docker, 3. Используются environment variables, 4. Реализована админ панель со всеми моделями(логин:admin, пароль:admin), 5. Проект развернут на удаленном сервере (ЯндексОблако) и доступен для тестировния, 6. Реализована модель Order, 7. Реализованы модели Discount, Tax, которые можно прикрепить к модели Order и связать с соответствующими атрибутами при создании платежа в Stripe - в таком случае они корректно отображаются в Stripe Checkout форме, 8.Реализовано поле Item.currency,и в зависимости от валюты выбранного товара предлагается оплата в соответствующей валюте.
